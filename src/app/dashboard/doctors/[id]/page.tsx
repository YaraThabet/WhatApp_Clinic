...
export default function DoctorDetailsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "Active":
        return (
          <Badge className="bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 border-none px-3 py-1">
            {t.dashboard.doctorsManagement.status.active}
          </Badge>
        );
      case "Inactive":
        return (
          <Badge className="bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 border-none px-3 py-1">
            {t.dashboard.doctorsManagement.status.inactive}
          </Badge>
        );
      case "On Leave":
        return (
          <Badge className="bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400 border-none px-3 py-1">
            {t.dashboard.doctorsManagement.status.onLeave}
          </Badge>
        );
      default:
        ...
        <div className="xl:col-span-2 space-y-8">
          <Tabs defaultValue="overview" className="w-full">
            <TabsList className="bg-white dark:bg-slate-900 p-1 rounded-2xl shadow-sm inline-flex mb-8 overflow-x-auto h-auto min-w-full sm:min-w-0">
              <TabsTrigger
                value="overview"
                className="rounded-xl px-6 py-2.5 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-600 dark:data-[state=active]:bg-blue-900/30 dark:data-[state=active]:text-blue-400"
              >
            ...
            <TabsContent value="overview">
              <Card className="border-none shadow-sm dark:bg-slate-900">
                <CardHeader>
                  <CardTitle>
                    {language === "ar" ? "عن الطبيب" : "Professional Biography"}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-8">
                    <div className="p-6 bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-slate-100 dark:border-slate-800">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">
                        {doctor.specialty} Specialist with focus on modern
                        diagnostic techniques and personalized treatment plans.
                      </p>
                    </div>
                    <div className="p-6 bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-slate-100 dark:border-slate-800">
                      <div className="flex items-center gap-3 mb-4">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">
                        Available for consultations during {doctor.workingHours}
                        . Accepts emergency appointments outside these hours.
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            <TabsContent value="appointments">
              <Card className="border-none shadow-sm dark:bg-slate-900">
                <CardHeader>
                  ...
                  <CardDescription>
                    {language === "ar"
                      ? "قائمة بجميع المواعيد المجدولة لهذا الطبيب"
                      : "List of all scheduled appointments for this doctor"}
                  </CardDescription>
                </CardHeader>
            ...
            <TabsContent value="patients">
              <Card className="border-none shadow-sm dark:bg-slate-900">
                <CardHeader>
                  <CardTitle>
                    {language === "ar" ? "المرضى المعالجون" : "Patient List"}
                  </CardTitle>
                </CardHeader>
                <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <TabsContent value="schedule">
              <Card className="border-none shadow-sm dark:bg-slate-900">
                ...
                <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-slate-500 dark:text-slate-400">
                    {language === "ar"
                      ? "تحميل الجدول الزمني للعيادة"
                      : "Load clinic timeline"}
                  </p>
                </CardContent>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="settings">
              <Card className="border-none shadow-sm dark:bg-slate-900">
                <CardHeader>
                  <CardTitle>
                    {language === "ar" ? "إعدادات الطبيب" : "Doctor Settings"}
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <p className="text-slate-500 dark:text-slate-400">
                    {language === "ar"
                      ? "إدارة تفضيلات الطبيب وصلاحيات الوصول"
                      : "Manage doctor preferences and access controls"}
                  </p>
                  <div className="flex flex-col gap-3">
                    ...
                    <Button
                      variant="outline"
                      className="justify-start gap-3 h-12 rounded-xl text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20"
                    >
